...
class ReportGenerator(object):
    def __init__(self, db, agency, debug=False):
        ...
        for domain_doc in all_domains_cursor:
            ...
            if domain_doc['is_base_domain']:
                domain_doc['subdomains'] = list(self.__db.trustymail.find({
                    'latest': True,
        ...
        logging.info('Retrieved {} mail domains for agency {}: {}'.format(
        ...
        for domain in self.__mail_domains:
            ...
            logging.info('Retrieved {} DMARC reports for domain {}'.format(
    ...
    def __score_domain(self, domain):
        ...
        if domain['live']:
            ...
            score['valid_dmarc'] = domain['valid_dmarc'] or \
                domain['valid_dmarc_base_domain']
            if score['valid_dmarc']:
                self.__valid_dmarc_count += 1

            # Placeholder for future use in reports.
            if domain['dmarc_results'] is None or \
               len(domain['dmarc_results']) == 0:
            ...
            # and include it.
            if domain['mail_servers'] is None or \
               len(domain['mail_servers']) == 0:
            ...
            if domain['domain_supports_smtp']:
                score['smtp_servers'] = [
                    s.strip()
                    for s in domain['domain_supports_smtp_results'].split(',')
                ]
                self.__domain_supports_smtp_count += 1

            # Does the domain support STARTTLS?
            score['domain_supports_starttls'] = domain[
            ...
            if domain.get('subdomains'):    # if this domain has any subdomains
                for subdomain in domain['subdomains']:
                    ...
                    if subdomain_score and (
                            subdomain['dmarc_record'] or
                            not domain['valid_dmarc']
                    ):
    ...
    def __generate_trustymail_attachment(self):
        header_fields = (
            'Domain', 'Base Domain', 'Domain Is Base Domain', 'Live',
            ...
            'DMARC Policy', 'DMARC Policy Percentage',
        ...
        data_fields = (
            'domain', 'base_domain', 'is_base_domain', 'live',
            'mx_record', 'mail_servers', 'mail_server_ports_tested',
            'domain_supports_smtp', 'domain_supports_smtp_results',
            'domain_supports_starttls', 'domain_supports_starttls_results',
            'spf_record', 'valid_spf', 'spf_results',
            'dmarc_record', 'valid_dmarc', 'dmarc_results',
            'dmarc_record_base_domain', 'valid_dmarc_base_domain',
            'dmarc_results_base_domain',
            'dmarc_policy', 'dmarc_policy_percentage',
            'dmarc_subdomain_policy',
            'aggregate_report_uris', 'forensic_report_uris',
            'has_aggregate_report_uri', 'has_forensic_report_uri',
            'syntax_errors', 'debug_info',
            'domain_has_weak_crypto',
            'hosts_with_weak_crypto_str')
        with open(TRUSTYMAIL_RESULTS_CSV_FILE, 'w') as out_file:
            header_writer = csv.DictWriter(out_file,
                                           header_fields,
                                           extrasaction='ignore')
            header_writer.writeheader()
            data_writer = csv.DictWriter(out_file,
                                         data_fields,
                                         extrasaction='ignore')
            ...
            for domain in self.__all_domains:
                ruas = [
                    rehydrate_rua_or_ruf(d)
                    for d in domain['aggregate_report_uris']
                ]
                rufs = [
                    rehydrate_rua_or_ruf(d)
                    for d in domain['forensic_report_uris']
                ]
    ...
    def __generate_dmarc_failures_attachment(self):
            ...
            for report in self.__dmarc_results[domain]:
                ...
                if isinstance(records, list):
                    failure_records = [
                        ...
                        for x in records if ReportGenerator.is_failure(x)
                    ]
                    records_to_save.extend(failure_records)
                elif isinstance(records, dict):
                    if ReportGenerator.is_failure(records):
                        records_to_save.append(
                            process_record(domain,
                                           records,
                                           policy_published)
                        )
        ...
        fields = (
            'DMARC Domain', 'Policy Applied', 'Override Reason', 'Count',
            'DKIM Alignment Result', 'DKIM Result', 'DKIM Domain',
            'SPF Alignment Result', 'SPF Result', 'SPF Domain',
            'Source IP', 'PTR', 'ASN'
        )
    ...
    def __generate_mustache_json(self, filename):
        ...
        result['valid_dmarc_reject_count'] = \
            self.__valid_dmarc_policy_of_reject_count
        result['valid_dmarc_reject_percentage'] = \
            self.__valid_dmarc_reject_percentage
        result['valid_dmarc_bod1801_rua_uri_count'] = \
            self.__valid_dmarc_bod1801_rua_uri_count
        result['valid_dmarc_bod1801_rua_uri_percentage'] = \
            self.__valid_dmarc_bod1801_rua_uri_percentage
        result['domain_supports_smtp_count'] = \
            self.__domain_supports_smtp_count
        ...
        result['supports_starttls_percentage'] = \
            self.__supports_starttls_percentage
        result['bod_1801_compliant_count'] = self.__bod_1801_compliant_count
        result['bod_1801_compliant_percentage'] = \
            self.__bod_1801_compliant_percentage
