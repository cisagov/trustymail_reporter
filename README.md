...
```console
docker run cisagov/trustymail_reporter:1.5.0-rc.2
```
...
1. Create a `docker-compose.yml` file similar to the one below to use [Docker Compose](https://docs.docker.com/compose/).
    ...
    services:
      trustymail_reporter:
        image: cisagov/trustymail_reporter:1.5.0-rc.2
        volumes:
          - type: bind
            source: <your_log_dir>
...
1. Pull the new image:
    ...
    ```console
    docker pull cisagov/trustymail_reporter:1.5.0-rc.2
    ```
...
|-----------|-------------|
|`cisagov/trustymail_reporter:1.5.0-rc.2`| An exact release version. |
...
```console
docker build \
  --tag cisagov/trustymail_reporter:1.5.0-rc.2 \
